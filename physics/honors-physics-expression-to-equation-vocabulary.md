...
| 14 | "horizontal range", "range", "how far does it land?" | $R = \frac{v_0^2\sin 2\theta}{g}$ |  **Level ground only.** If launched from a cliff/tower/table (landing height ≠ launch height), DO NOT use this. Use two-step: $t_{\text{fall}} = \sqrt{2h/g}$ or $\Delta y = v_{0y}t - \frac{1}{2}gt^2$, then $R = v_x t$. |
