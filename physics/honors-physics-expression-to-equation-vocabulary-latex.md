---
title: "The Physics Reflex — Expression → Equation Flashcard Deck"
author: ""
date: ""
...
| 32 [M] | "coefficient of friction $\mu$" | Dimensionless. $\mu_s > \mu_k$ always. | Typically $0 < \mu < 1$, but $\mu > 1$ is possible (e.g., rubber on rubber). |
...
| 99 [M] | "geostationary orbit" | $T = 24\;\mathrm{h}$; solve $r$ from Kepler's 3rd Law | $r \approx 42{,}000\;\mathrm{km}$ from Earth's center. |
| 100 [★★] | "escape velocity", "escape speed" | $v_{\text{esc}} = \sqrt{\frac{2GM}{R}}$ | $v_{\text{esc}}$ from Earth: ~$11.2\;\mathrm{km/s}$. |
| 101 [M] | "gravitational potential energy" (universal) | $U = -G\frac{Mm}{r}$ | Zero at infinity. Always negative for bound systems. |
...
|---|-------------|-----|-----------|
| 106 [M] | Point mass at distance $r$ | $I = mr^2$ | The definition. Everything builds from this. |
| 107 [M] | Solid cylinder / disk (about center) | $I = \frac{1}{2}MR^2$ | Most common rotational object on exams. |
| 108 [M] | Solid sphere (about center) | $I = \frac{2}{5}MR^2$ | A sphere rolls faster than a cylinder down an incline. |
| 109 [M] | Hollow sphere / spherical shell | $I = \frac{2}{3}MR^2$ | More inertia than solid sphere → slower rolling. |
| 110 [M] | Thin rod, axis through center | $I = \frac{1}{12}ML^2$ | Door rotating about its hinge uses the end-axis version. |
| 111 [M] | Thin rod, axis through end | $I = \frac{1}{3}ML^2$ | |
...
| 150 [M] | "speed of sound in air" | $v \approx 343\;\mathrm{m/s}$ at $20^\circ\mathrm{C}$; $v = 331\sqrt{1 + T_C/273}$ | Increases with temperature. For the exact derivation from first principles, see Card 354 ($v = \sqrt{\gamma RT/M}$). |
...
| 155 [M] | "permittivity of free space $\varepsilon_0$" | $k = \frac{1}{4\pi\varepsilon_0}$, $\varepsilon_0 = 8.85\times10^{-12}\;\mathrm{C^2/(N\;m^2)}$ | |
...
| 165 [M] | "electron volt", "eV" | $1\;\mathrm{eV} = 1.60\times10^{-19}\;\mathrm{J}$ | Energy unit. The kinetic energy gained by one electron accelerated through 1 V. |
...
| 189 [M] | "time constant $\tau = RC$" | After $1\tau$: 63% charged. After $5\tau$: ~99% charged. | $\tau$ has units of time. Verify: $\Omega \times \mathrm{F} = (V/A) \times (C/V) = C/A = \mathrm{s}$. |
...
| # | Trigger Phrase | Instant Equation | Exam Note |
|---|---------------|------------------|-----------|
| 199 [M] | "permeability of free space $\mu_0$" | $\mu_0 = 4\pi\times10^{-7}\;\mathrm{T\;m/A}$ | The magnetic analog of $\varepsilon_0$. |
...
| 212 [M] | "rms value" (AC) | $V_{\text{rms}} = \frac{V_0}{\sqrt{2}}$, $I_{\text{rms}} = \frac{I_0}{\sqrt{2}}$ | The DC equivalent that delivers the same power. |
...
| 218 [M] | "Avogadro's number $N_A$" | $N_A = 6.022\times10^{23}\;\mathrm{mol^{-1}}$ | Number of particles in one mole. |
| 219 [M] | "Boltzmann constant $k_B$" | $k_B = \frac{R}{N_A} = 1.38\times10^{-23}\;\mathrm{J/K}$ | Gas constant per particle. $PV = Nk_B T$. |
...
| 229 [M] | Water: heat of fusion | $L_f = 3.34\times10^5\;\mathrm{J/kg}$ | Ice melting at 0°C. |
| 230 [M] | Water: heat of vaporization | $L_v = 2.26\times10^6\;\mathrm{J/kg}$ | Water boiling at 100°C. |
...
| 236 [M] | "adiabatic index $\gamma$" | $\gamma = C_P/C_V$; monatomic: $\gamma = 5/3$; diatomic: $\gamma = 7/5$ | |
...
| 269 [M] | Sign convention summary | $f > 0$: concave mirror, converging lens. $f < 0$: convex mirror, diverging lens. | $d_i > 0$: real image (light actually converges there). $d_i < 0$: virtual image. |
...
| # | Trigger Phrase | Instant Equation | Exam Note |
|---|---------------|------------------|-----------|
| 283 [M] | "Planck's constant $h$" | $h = 6.626\times10^{-34}\;\mathrm{J\;s}$ | The quantum of action. |
...
| 293 [M] | "Lyman series" | $n_f = 1$ (UV) | Transitions to ground state. |
| 294 [M] | "Balmer series" | $n_f = 2$ (visible) | Transitions to first excited state. |
| 295 [M] | "Paschen series" | $n_f = 3$ (infrared) | Transitions to second excited state. |

### 15.3 Special Relativity

| # | Trigger Phrase | Instant Equation | Exam Note |
|---|---------------|------------------|-----------|
| 296 [M] | "speed of light $c$" | $c = 3.00\times10^8\;\mathrm{m/s}$ | The universal speed limit. Same in all inertial frames. |
...
| 299 [M] | "Lorentz factor $\gamma$" | $\gamma = \frac{1}{\sqrt{1 - v^2/c^2}}$ | $\gamma \ge 1$. Approaches $\infty$ as $v \to c$. |
...
| 377 | atm | Pa | $1\ \mathrm{atm} = 1.013 \times 10^{5}\ \mathrm{Pa}$ |
| 378 | atm | mmHg (torr) | $1\ \mathrm{atm} = 760\ \mathrm{mmHg}$ |
| 379 | L | m³ | $1\ \mathrm{L} = 10^{-3}\ \mathrm{m}^{3}$ |
| 380 | eV | J | $1\ \mathrm{eV} = 1.60 \times 10^{-19}\ \mathrm{J}$ |
| 381 | cal | J | $1\ \mathrm{cal} = 4.186\ \mathrm{J}$ |
| 382 | kWh | J | $1\ \mathrm{kWh} = 3.6 \times 10^{6}\ \mathrm{J}$ |
| 383 | rpm | rad/s | Multiply by $2\pi/60$ |
| 384 | km/h | m/s | Divide by $3.6$ |
| 385 | $\mu$C | C | $10^{-6}$ |
| 386 | nC | C | $10^{-9}$ |
| 387 | gauss | tesla | $1\ \mathrm{G} = 10^{-4}\ \mathrm{T}$ |
| 388 | atomic mass unit (u) | kg | $1\ \mathrm{u} = 1.661 \times 10^{-27}\ \mathrm{kg}$ |
| 389 | atomic mass unit (u) | MeV/$c^{2}$ | $1\ \mathrm{u} = 931.5\ \mathrm{MeV}/c^{2}$ |
| 390 | Å (angstrom) | m | $1\ \text{Å} = 10^{-10}\ \mathrm{m}$ |
...
|---|----------|--------|---------|------------------|
| 394 [M] | Mass | $[M]$ | kilogram (kg) | Amount of matter |
| 395 [M] | Length | $[L]$ | meter (m) | Distance, position, displacement |
| 396 [M] | Time | $[T]$ | second (s) | Duration, period |
| 397 [M] | Electric Current | $[I]$ or $[Q]$ | ampere (A) or coulomb (C) | Flow of charge (we use $[Q]$ for charge-based analysis) |
| 398 [M] | Temperature | $[\Theta]$ | kelvin (K) | Thermal energy level |
| 399 [M] | Amount of Substance | $[N]$ | mole (mol) | Number of particles |
| 400 [M] | Luminous Intensity | $[J]$ | candela (cd) | Brightness (rarely used in Honors Physics) |
